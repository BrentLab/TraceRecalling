...
sub new {
	...
	$this->{FH} = $fh;
	$this->{LASTLINE} = "";
	if (my $status = $this->_parseHeader) {
		# there are alignments
		$this->{REPORT_DONE} = 0;
		if ($status == 64) { # empty report
			return $status;
		}
...
sub map_est_to_genome {
	my ($this, $pos) = @_;
	my $element_ref = $this->{ELEMENTS};	
	my @elements =  @$element_ref;
	if ($#elements < 0) {
		return -1;
	}
	if ($pos < $this->eb || $pos > $this->ee) {
		print "$pos in est is not found in alignment\n";
		return -1;
	}
	foreach my $element (@elements) {
		if ($pos >= $element->eb && $pos <= $element->ee) {
			return $element->map_est_to_genome($pos);
		}
	}
	print "I shouldn't be here. Tell Mani!\n";
	return -1;
}

sub map_genome_to_est {
	my ($this, $pos) = @_;
	my $element_ref = $this->{ELEMENTS};	
	my @elements =  @$element_ref;
	if ($#elements < 0) {
	...
	if ($pos < $this->gb || $pos > $this->ge) {
		print "$pos in genome is not found in alignment\n";
		return -1;
	}
	foreach my $element (@elements) {
		if ($pos >= $element->gb && $pos <= $element->ge) {
			return $element->map_genome_to_est($pos);
		}
	}
	print "I shouldn't be here. Tell Mani!\n";
	return -1;
...
sub _parseHeader {
	...
	while(<$FH>) {
		if ($_ =~ /Note Best alignment is between (reversed|forward) est and (reversed|forward) genome, (but|and) splice\s+sites imply\s+(forward gene|REVERSED GENE)/ ) {
			...
			push @exons, $exon;
			push @elements, $exon;
			$this->{LAST_EST_BASE} = $ee; # Track intron position in EST coordinates
		}
		elsif ($_ =~ /^Span\s/)    {
			...
			my ($eb, $ee) = ($this->{LAST_EST_BASE} + 1, $this->{LAST_EST_BASE});
			...
			$this->{REPORT_DONE} = 1;
			$this->{SCORE} = $1;
			return 64; # there's nothing in the report
		}
	}
...
#print "In genome";
	...
	# handle lines in sets of 3: genome, alignment, est
...
sub map_est_to_genome {
	my ($element, $pos) = @_;
	if ($element->type ne "EXON") {
		print "Cannot map an intron from est coordinates to genome coordinates\n";
		return -1;
	}
	my $offset = $pos - $element->eb;
	my @genome = split(//, $element->ga);
	my @est    = split(//, $element->ea);
	my $moved = -1;
	my $upper_bound = -1;
	if ($pos < $element->eb || $pos > $element->ee) {
		print "$pos Out of range:".$element->est."\n";
		return -1;
	}
	for (my $i = 0; $i < scalar(@est); $i++) {
		...
		if ($moved == $offset) {
			$upper_bound = $i;
			$moved       = $i;
			#print "moved=$moved; upper_bound=$upper_bound; est=$est[$moved]\n";
			last; # Hit the position
		}
	}
	if ($upper_bound == -1) {
	...
	for (my $j = 0; $j <= $upper_bound ; $j++) {
		if ($genome[$j] eq "-") {
			$moved--;
		}
	}
	#print "moved=$moved; upper_bound=$upper_bound; genome=$genome[$moved]\n";
	if ($genome[$upper_bound] eq "-") {
		$moved += 0.5;
		warn "$element: EST POSITION $pos MAPS TO A GAP IN GENOME\n";
	}
	return ($element->gb + $moved);
}

sub map_genome_to_est {
	my ($element, $pos) = @_;
	if ($element->type ne "EXON") {
		print "Cannot map $pos in intron from genome coordinates to est coordinates\n";
		return -1;
	}
	my $offset = $pos - $element->gb;
	my @genome = split(//, $element->ga);
	my @est    = split(//, $element->ea);
	my $moved = -1;
	my $upper_bound = -1;
	if ($pos < $element->gb || $pos > $element->ge) {
		print "$pos Out of range:".$element->genome."\n";
		return -1;
	}
	...
	#print "moved=$moved; upper_bound=$upper_bound; genome=$genome[$moved]\n";
	if ($est[$upper_bound] eq "-") {
		$moved += 0.5;
		warn "$element: GENOME POSITION $pos MAPS TO A GAP IN EST\n";
	}
	return ($element->eb + $moved);
}
...
=head1 SYNOPSIS
 ...
 $report->ee;
 $report->hasIntron;
 $report->countElements;
 $report->map_est_to_genome;
 ...
 while(my $element = $report->nextElement) {
     ...
     $element->score;
     $element->pi;
     $element->map_est_to_genome;# Map a base in EST (inside this element) to genomic coordinate 
                                 # (Returns n+0.5 if the EST base is in a genomic gap after n)
 }
...
The report has many attributes and methods.
 ...
 $report->map_est_to_genome;# Map a base in EST to genomic coordinate 
